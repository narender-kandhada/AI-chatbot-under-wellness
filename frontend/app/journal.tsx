                ...
                <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}>
                    {entries.length === 0 ? (
                        <View style={styles.emptyState}>
                            ...
                            <Text style={[styles.emptyTitle, { color: theme.textSecondary }]}>No entries yet</Text>
                                ...
                                >
                                    <View style={styles.entryHeader}>
                                        ...
                                        <View style={styles.entryMeta}>
                                            ...
                                            <Text style={[styles.entryTime, { color: theme.textLight }]}>{formatTime(entry.date)}</Text>
                                        </View>
                                    ...
                                    {isExpanded && (
                                        <View style={[styles.messagesWrap, { borderTopColor: theme.border }]}>
                                            {entry.messages.filter(m => m.id !== '1').map((msg, i) => (
                                                <View key={i} style={[
                                                    styles.msgBubble,
                                                    msg.isUser ? styles.msgUser : styles.msgAi,
                                                    {
                                                        backgroundColor: msg.isUser ? theme.primary + '15' : theme.surfaceTint,
                                                    }
                                                ]}>
...
const styles = StyleSheet.create({
    ...
    emptyText: { fontSize: typography.sizes.base, textAlign: 'center', lineHeight: typography.sizes.base * 1.6, paddingHorizontal: spacing.xl },
    entryCard: {
    ...
    entryMeta: { flex: 1 },
    entryDate: { fontSize: typography.sizes.base, fontWeight: '700' },
    entryTime: { fontSize: typography.sizes.xs },
    entryMsgCount: { fontSize: typography.sizes.xs },
    entryPreview: { fontSize: typography.sizes.sm, lineHeight: typography.sizes.sm * 1.5 },
    messagesWrap: { marginTop: spacing.md, paddingTop: spacing.md, borderTopWidth: 1, gap: spacing.sm },
    msgBubble: { padding: spacing.sm, borderRadius: borderRadius.md },
    msgUser: { marginLeft: spacing.xl },
    msgAi: { marginRight: spacing.xl },
    msgRole: { fontSize: typography.sizes.xs, fontWeight: '700', marginBottom: 2 },
    msgText: { fontSize: typography.sizes.sm, lineHeight: typography.sizes.sm * 1.5 },
});
